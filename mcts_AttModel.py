...
bad_endings = [
    'a', 'an', 'the', 'in', 'for', 'at', 'of', 'with', 'before', 'after', 'on',
    'upon', 'near', 'to', 'is', 'are', 'am'
]
bad_endings += ['the']
...
def sort_pack_padded_sequence(input, lengths):
    ...
    #debug - end
    tmp = pack_padded_sequence(input[indices],
                               sorted_lengths,
                               batch_first=True)
...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.seq_length = getattr(
            opt, 'max_length', 20) or opt.seq_length  # maximum sample length
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        outputs = fc_feats.new_zeros(batch_size * seq_per_img, seq.size(1),
                                     self.vocab_size + 1)
    ...
    def _old_sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        seq = fc_feats.new_full((batch_size * sample_n, self.seq_length),
                                self.pad_idx,
                                dtype=torch.long)
        ...
        for k in range(batch_size):
            ...
            for t in range(1):
                if t == 0:  # input <bos>
                    it = fc_feats.new_full([beam_size],
                                           self.bos_idx,
                                           dtype=torch.long)
    ...
    def _prefix_next(self, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks,
        ...
        seq_probs = p_att_feats.new_zeros(batch_size, max_len + 1,
                                          self.vocab_size + 1)
        seq_values = p_att_feats.new_zeros(batch_size, max_len + 1)

        # input <bos>
        fetch_values = list()
        logits, state = self.get_logprobs_state(
    ...
    def _extend_trajectory(self, p_fc_feats, p_att_feats, pp_att_feats,
        ...
        for t in range(self.seq_length + 1):
            if t == 0:  # input <bos>
                it = p_att_feats.new_full([batch_size],
                                          self.bos_idx,
                                          dtype=torch.long)
    ...
    def _diverse_sample(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        seq_table = [
            fc_feats.new_full((batch_size, self.seq_length),
                              self.pad_idx,
                              dtype=torch.long) for _ in range(group_size)
        ]
        seqLogprobs_table = [
            fc_feats.new_zeros(batch_size, self.seq_length)
        ...
        for tt in range(self.seq_length + group_size):
            for divm in range(group_size):
                ...
                if t >= 0 and t <= self.seq_length - 1:
                    if t == 0:  # input <bos>
                        it = fc_feats.new_full([batch_size],
                                               self.bos_idx,
                                               dtype=torch.long)
                    else:
                        it = seq[:, t - 1]  # changed
                    ...
                    if divm > 0:
                        ...
                        for prev_choice in range(divm):
                            ...
                            logprobs[:,
                                     prev_decisions] = logprobs[:,
                                                                prev_decisions] - diversity_lambda

                    if decoding_constraint and t > 0:
                        tmp = logprobs.new_zeros(logprobs.size())
                        tmp.scatter_(1, seq[:, t - 1].data.unsqueeze(1),
                                     float('-inf'))
                        logprobs = logprobs + tmp

                    if remove_bad_endings and t > 0:
                        tmp = logprobs.new_zeros(logprobs.size())
                        prev_bad = np.isin(seq[:, t - 1].data.cpu().numpy(),
                                           self.bad_endings_ix)
                        # Impossible to generate remove_bad_endings
                        tmp[torch.from_numpy(prev_bad.astype('uint8')),
                            0] = float('-inf')
                        logprobs = logprobs + tmp
                    ...
                    if block_trigrams and t >= 3:
                        ...
                        mask = torch.zeros(logprobs.size(),
        ...
        return torch.stack(seq_table, 1).reshape(
            ...
            -1), torch.stack(seqLogprobs_table,
                             1).reshape(batch_size * group_size, -1)
...
class AdaAtt_lstm(nn.Module):
    ...
    def forward(self, xt, img_fc, state):
        ...
        for L in range(self.num_layers):
            ...
            out_gate = sigmoid_chunk.narrow(1, self.rnn_size * 2,
                                            self.rnn_size)
        ...
        state = (torch.cat([_.unsqueeze(0) for _ in hs],
                           0), torch.cat([_.unsqueeze(0) for _ in cs], 0))
...
class AdaAtt_attention(nn.Module):
    def __init__(self, opt):
        ...
        self.fr_linear = nn.Sequential(
            nn.Linear(self.rnn_size, self.input_encoding_size), nn.ReLU(),
            nn.Dropout(self.drop_prob_lm))
        self.fr_embed = nn.Linear(self.input_encoding_size, self.att_hid_size)

        # h out embed
        self.ho_linear = nn.Sequential(
            nn.Linear(self.rnn_size, self.input_encoding_size), nn.Tanh(),
            nn.Dropout(self.drop_prob_lm))
    ...
    def forward(self,
        ...
        img_all_embed = torch.cat([
            fake_region_embed.view(-1, 1, self.input_encoding_size),
            conv_feat_embed
        ], 1)
        ...
        if att_masks is not None:
            ...
            PI = PI * torch.cat([att_masks[:, :1], att_masks],
                                1)  # assume one one at the first time step.
...
class UpDownCore(nn.Module):
    ...
    def forward(self,
        ...
        h_att, c_att = self.att_lstm(att_lstm_input,
                                     (state[0][0], state[1][0]))
...
class Attention(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats, att_masks=None):
        ...
        att_feats_ = att_feats.view(
            -1, att_size,
            att_feats.size(-1))  # batch * att_size * att_feat_size
...
class LMModel(AttModel):
    def __init__(self, opt):
        ...
        delattr(self, 'fc_embed')
        self.fc_embed = lambda x: x.new_zeros(x.shape[0], self.
                                              input_encoding_size)
        self.embed = nn.Embedding(self.vocab_size + 1,
                                  self.input_encoding_size)
        self._core = LSTMCore(opt)
